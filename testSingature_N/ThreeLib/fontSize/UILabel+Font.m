//
//  UILabel+Font.m
//
//  Created by ljf on 2022/6/16.
//  Copyright ljf. All rights reserved.
//

#import "UILabel+Font.h"
#import <objc/runtime.h>
#import "FontConfig.h"
#import "YYText.h"

@implementation UILabel (Font)

+ (void)load{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [self swizzledSel:@selector(setFont:) withSel:@selector(swizzled_setFont:)];
        [self swizzledSel:@selector(setAttributedText:) withSel:@selector(swizzled_setAttributedText:)];
        [self swizzledSel:@selector(awakeFromNib) withSel:@selector(swizzled_awakeFromNib)];
    });
}

+ (void)swizzledSel:(SEL)originalSelector withSel:(SEL)swizzledSelector{
    Class class = [self class];
    Method originalMethod = class_getInstanceMethod(class, originalSelector);
    Method swizzledMethod = class_getInstanceMethod(class, swizzledSelector);
    
    if (!class_addMethod(class, originalSelector, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod))) {
        method_exchangeImplementations(originalMethod, swizzledMethod);
    } else {
        class_replaceMethod(class, swizzledSelector, method_getImplementation(originalMethod), method_getTypeEncoding(originalMethod));
    }
}

- (void)swizzled_setFont:(UIFont *)font{
    UIFont *newFont = [FontConfig getScaleFont:font];
    [self swizzled_setFont:newFont];
}

- (void)swizzled_awakeFromNib {
    self.font = [FontConfig getScaleFont:self.font];
    [self swizzled_awakeFromNib];
}

- (void)swizzled_setAttributedText:(NSAttributedString *)attributedText{
    NSMutableAttributedString *newAttributedText = [[NSMutableAttributedString alloc]initWithAttributedString:attributedText];
    UIFont *font = [newAttributedText.yy_attributes valueForKey:NSFontAttributeName];
    if (font) {
        font = [FontConfig getScaleFont:font];
        newAttributedText.yy_font = font;
    }
    [self swizzled_setAttributedText:newAttributedText];
}

@end
