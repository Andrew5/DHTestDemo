...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (scrollView==self.myTableView) {
        CGPoint gjPoint  = CGPointMake(100, 44+44+self.titleSV.frame.size.height+1);//+1很重要，+1确保关键的点在需要的位置
...
- (void)viewDidLoad {
    ...
    self.myTableView  = [[UITableView alloc]initWithFrame:CGRectMake(0, 44+44+self.titleSV.frame.size.height, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 49-34-self.titleSV.frame.size.height) style:UITableViewStylePlain];
    ...
    self.myTableView.contentInset = UIEdgeInsetsMake(0, 0, [UIScreen mainScreen].bounds.size.height-self.titleSV.frame.size.height-44-44-49-34, 0);
