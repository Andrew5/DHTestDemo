...
- (void)autoScroll
{
    ...
    __block UIScrollView *tmpView = _scrollView;
    __block NSMutableArray *tmpImages = _images;
    [UIView animateWithDuration:0.8 animations:^{
        [tmpView setContentOffset:CGPointMake(2 * tmpView.bounds.size.width, 0)];
    } completion:^(BOOL finished) {
        [self->_middleImageView setImageViewContent:tmpImages[self->_currentIndex]];
        [self->_rightImageView setImageViewContent:tmpImages[rightIndex]];
        [self->_leftImageView setImageViewContent: tmpImages[leftIndex]];
        
        [tmpView setContentOffset:CGPointMake(self.bounds.size.width, 0) animated:NO];
        
        if (self.didScrollToIndexBlock)
        {
            self.didScrollToIndexBlock(self->_currentIndex);
        }
    }];
